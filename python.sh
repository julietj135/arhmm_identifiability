#!/bin/bash
#SBATCH --job-name=two-states             # Job name
#SBATCH --mem=60000                     # Job memory request
#SBATCH -t 0-00:20:00               # Time limit days-hrs:min:sec
#SBATCH -N 1                         # requested number of nodes (usually just 1)
#SBATCH -p scavenger-gpu             # requested partition on which the job will run
#SBATCH --gres=gpu:1 
#SBATCH --exclusive
#SBATCH --output=outputs/TWO-STATES/two-states.out   # file path to slurm output
