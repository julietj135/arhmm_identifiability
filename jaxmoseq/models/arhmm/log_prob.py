...
def marginal_log_likelihood(mask, x, Ab, Q, pi, **kwargs):
    ...
    log_likelihoods = jax.lax.map(partial(ar_log_likelihood, x), (Ab, Q))
    log_likelihoods = jnp.moveaxis(log_likelihoods, 0, -1) # log(P(y | z,u,theta))
    masked_log_likelihoods = log_likelihoods * mask[:, nlags:, None] 
    # with continuous state space, log of density, so can be negative or positive
    # forward algorithm, finds log(P(state seq | params))
